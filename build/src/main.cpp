...
int main(int argc, char *argv[]) {
    ...
    while (!close) {
        ...
        if(frame % blobMajorityMessageDelay == 0 && showMajorityMessage){
            int red = 0;
            int green = 0;
            int blue = 0;
            ...
            int cyan = 0;
            for (int i = 0; i < blobs.size(); i++) {
                if(blobs[i].r > 200 && blobs[i].g < 50 && blobs[i].b < 50) {
                    red++;
                }
                if(blobs[i].r < 50 && blobs[i].g > 200 && blobs[i].b < 50) {
                    green++;
                }
                if(blobs[i].r < 50 && blobs[i].g < 50 && blobs[i].b > 200) {
                    blue++;
                }
                if(includeYPC){
                if(blobs[i].r > 200 && blobs[i].g > 200 && blobs[i].b < 50) {
                    yellow++;
                }
                if(blobs[i].r > 200 && blobs[i].g < 50 && blobs[i].b > 200) {
                    purple++;
                }
                if(blobs[i].r < 50 && blobs[i].g > 200 && blobs[i].b > 200) {
                    cyan++;
                }
                }
            }
